...
| `money-compare` | `left`, `right`, `expected`, `spec`, `note?` | compare two decimal strings exactly (§01 §2.5); `expected` is `less`, `equal`, `greater`, or `refused` — **not** `-1`/`0`/`1`, which is what this row said until 2026-08-04 and what the file has never carried (row B4). A harness written from the old text failed all 31 vectors. |
