...
| DEF-16 | closed | implementation defect | high — the refusal is false | With the kernel unreachable, a `consequential` call returns `result: parked` with a request hash the kernel then 404s. Nothing is queued, no human will ever see it, and the agent believes it is pending. §05 §7.1 has an `unreachable` outcome and this path reports the wrong one. Found by the commerce partner (`docs/validation/design-partners/commerce.md`). **Verified here 2026-08-05, and the report is half wrong in the system's favour.** The state is real: with no kernel, nothing is queued and the hash 404s. But the gateway does *not* stay silent — `_queue_with_kernel` returns the reason and the refusal's `hint` carries it verbatim: *"held locally; the kernel was unreachable, so nothing was queued for a human to see"*. What is true is narrower and still a defect: `result` reads `parked` when nothing is parked anywhere a human can reach, and `result` is what a caller keys on. The fix is a reason code, and §06's codes are contractual — so it lands with vectors, not on the day it was verified. **Closed 2026-08-05, and not by relabelling the state.** `parked` was optimistic rather than false: the request is held, and `_queue_with_kernel` re-submits it — but only when the *caller* repeats the call, and §06 §4.1 asks a well-behaved agent to report a terminal answer and stop. So the only exit depended on the agent misbehaving. `Enforcer.requeue_parks` now offers held parks at session open, beside `recover_intents` and `apply_pending_seeds`, and fires no notification — a ping per reconnect is the approval fatigue §09 §7 calls an availability attack. It also carries the arguments: §06 §4.4 rule 7 makes the first accepted submission's values the recorded ones, so a re-offer without them would land a permanently blank row, which is worse than the invisibility. That is what the gateway store's **first real migration** (version 2) is for. `gateway/tests/test_parks_reach_the_queue.py`, three cases including the control that an answered park is not re-offered. |
| DEF-17 | closed | implementation defect | medium — "never existed" and "lawfully deleted" are indistinguishable | `/v1/payloads/<hash>` returns byte-identical `410 decayed` for a hash that has never existed. An auditor cannot tell a payload that was retained and expired from one that was never recorded, and the export does not mark decayed evidence. Found by the clinical partner (`docs/validation/design-partners/clinical.md`). **Closed 2026-08-05, and it needed no new bookkeeping.** `payload_refs` is written when an envelope commits to a payload and is **never deleted** — the sweep touches `payloads` alone, which is what keeps chain verification unaffected. The record of "this was recorded once" already outlived the bytes and nothing was reading it. A hash no envelope ever cited is now `404 unknown`; a decayed one stays `410 decayed`. This is load-bearing for the retention claim itself: *"closed loops decay to signed hashes"* only means something if `410` says the commitment exists. `kernel/stozher-kernel/tests/decayed_is_not_unknown.rs`, which fails when the tombstone lookup is made to miss, and carries the control that the payload was stored before the sweep. |
| DEF-18 | closed | not an implementation defect | high for adoption | The rate limiter **drops** consequential work instead of queueing it: 66 of 93 gated calls in one simulated morning were refused `gate-rate-limited` with `retryable: false`. The refund never happens. The cap is deliberate and stays. **`retryable: false` was not deliberate — it fell out of `retryable = status == 503`**, so a 429 told the caller never to try again for a condition that is transient by definition, and the queue test's own comment already said *"the window is a window"*. **Closed 2026-08-05**: a 429 is retryable and carries `retry-after-seconds`, and the two travel together or neither does — `refusal.py`'s existing argument is right that a caller told "yes" with no "when" hammers the gate, which is how the cap filled. Bound on both sides separately: the kernel's window in `gate_queue_and_console_decisions.rs::one_subject_cannot_grow_the_queue_without_bound`, the pass-through and the no-window control in `test_enforcement.py`. What is still *not* fixed and is the design question this row was opened for: an organization steadily above the cap has no queue to fall into, only a slower loop. |
...
| DEF-20 | closed | not an implementation defect | medium | `gate-rules` members are closed to `["classes", "decision", "approvers"]`: no per-action approver, no quorum. A firm cannot say "a partner approves filings, an associate approves everything else". Found by the legal partner. **The approver half closed 2026-08-05**: `spec/05 §3.2` defines `gate-rules[].actions` in §03 §4.1's pattern vocabulary, first match wins in document order — deliberately *not* scored for specificity like `reclassify`, because a policy author writes the narrow rule first and reads top to bottom, and a score would silently reorder their intent. Five vectors in `policy-evaluation.json`. **The two implementations disagreed on the day it was added** — over what `*` means as a *query* action — which is exactly the divergence class this corpus exists for; the spec now states it and a vector asks both. **Quorum is not closed and is DEF-22**: it changes the shape of `authorization`, which carries one `decision`, and that is a different change. |
| DEF-23 | closed | implementation defect, **introduced by DEF-16's fix** | critical — the gateway served its tools ungoverned | `requeue_parks` let one park's refusal out of the loop. `_queue_with_kernel` raises on a kernel refusal by design — right for a fresh park, where the request is in no queue and calling it `parked` would be a lie — and propagating from session open it meant **enforcement mode did not start at all**. Found within an hour, by installing this on a real deployment: a park from five days earlier was re-offered and refused `gate-request-expired`, correctly, because a request's `not-after` is one hour. One stale row became a total outage of the thing that governs, which is worse than the invisibility DEF-16 was about. **Closed 2026-08-05**, same day it was introduced: the refusal is caught per park and the row is left for the next session. `gateway/tests/test_parks_reach_the_queue.py`. A recovery loop must never be able to stop a session from starting. |
| DEF-22 | design question | not an implementation defect | medium | **Quorum: N approvers for one action.** Split out of DEF-20 on 2026-08-05 rather than closed with it, because it is not the same change. §06 §1's `authorization` carries one `request` and one `decision`; N signatures needs a member that holds N, a rule for what counts as distinct (subject, not key — §06 §5 already says a human with two keys is one human), and a decision about whether a partial quorum is a durable state or is discarded. Vectors on both sides. The legal partner asked for it beside the per-action approver, and only the second was cheap. |
| DEF-21 | closed | not an implementation defect | medium | An envelope has no matter/case/tenant dimension, so "what did the agent do on the Acme matter?" is unanswerable from the stream. Related to ADR-0034's second-dimension question but not the same: this one is about *grouping* rather than about *scope*. Found by the legal partner. **Closed 2026-08-05, and the report was wrong about the protocol and right about the product.** `spec/02` has carried `correlation-ref` — *"stored and indexed, never interpreted"* — all along, and the kernel serves `?correlation-ref=` and `?correlation-prefix=` over it. **The gateway never set it.** So the dimension was real, queryable, and unreachable, which is indistinguishable from absent to the person asking. `gateway.correlation_ref` now stamps every emitted effect; one process serves one caller on one device, so that is the grain the question has. Per-call was rejected: it would travel inside the tool's own arguments and contaminate both what the approver reads and what `args-hash` commits to. `gateway/tests/test_correlation_ref.py`, with the control that an unconfigured gateway stamps nothing — an empty matter is worse than none. |
