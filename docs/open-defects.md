...
| DEF-12 | observed | **security defect**, high | high — `object-hash` collision surface | `serde_json` without `float_roundtrip` parses some binary64 values to a neighbouring float, so the kernel and the gateway compute **different `object-hash` values for 5.3% of a random document corpus**, and two distinct documents can share one hash in the kernel. Reachable: payload bodies are hashed with no numeric restriction, and the reviewer had a payload accepted under the hash of a different payload. External review Finding 2 (`docs/validation/security-review-2026-08-04.md`); the reviewer calls it a one-line fix. Not applied here because changing a canonicalizer changes every hash it has ever produced, and that decision wants its own change with vectors on both sides. |
| DEF-13 | observed | **security defect**, medium | medium | The gateway accepts confusable non-ASCII-digit timestamps that the kernel refuses, and **six of them never expire**. External review Finding 3 (`docs/validation/security-review-2026-08-04.md`), `gateway/src/stozher_gateway/envelope.py`. A divergence in what the two halves accept, on the member that decides expiry. |
