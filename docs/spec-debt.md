...
| B2 | ~~**§06 §2 declares its eleven steps complete and they are not.**~~ **Paid 2026-08-04.** The algorithm gains a **step (0)** — member sets and the *shape* of `not-after` / `decided-at` / `emitted-at`, checked before steps (8) and (9) compare them as strings — and §1.2 gains the rule that a non-boolean or absent `single-use` is read as `true`, with the reason: the restrictive reading of a malformed value costs an approver a second signature, the permissive one costs the organization a second effect. §3's cross-reference to "step (9)" is corrected to (11). **Four vectors added** (`authorization.json`, 19 → 23) and both implementations passed them unchanged — the agreement was real, only unbound. The original text: Nothing validates the *shape* of `not-after` / `decided-at` before steps (8) and (9) compare them as strings, so a verifier built exactly to the algorithm accepts an approval whose `not-after` is `"z"` and never expires. | `grep -n "encoding-bad-timestamp" spec/06-gates.md`: **zero hits**. The steps do compare timestamps as strings. | **Yes** — an approval that never expires is a security property, not a formatting one. |
| B3 | ~~**No error-code precedence rule exists**~~ **Paid 2026-08-04.** §00 §1 gains it: the **most specific** code wins — a code naming a particular member on a particular kind over one naming a class of members over a generic structural code, ties to the earlier section. The `cognition` case is named outright, since `envelope-shape.json` already pins both halves of it. Documented rather than decided: the corpus enforced this in one place and the rule was nowhere. The original text:, over roughly ninety normative codes. §02 §2.1 and §02 §9.1 give different codes for the same input and the text never says which wins. Two implementations that disagree here disagree *on the wire*, since §00 §1 makes codes part of the contract. | The auditor lost four vectors to it. The collision is in the text as described. | **Yes** for interoperability, no for safety. |
| B4 | ~~**Monetary comparison has three incompatible spellings of its own outcome**~~ **Paid 2026-08-04.** `spec/vectors/README.md` claimed `expected` is `-1`/`0`/`1`; the file has always carried `less`/`equal`/`greater`/`refused`. The README was wrong about its own corpus, and is corrected in place with what it used to say — a harness written from it failed all 31 vectors, which is exactly what happened to the blind implementer. The original text: — `spec/03 §4.3` (`schema-type-mismatch`), `vectors/README.md` §3 (`-1\|0\|1`), and `money-compare.json` (`less\|equal\|greater\|refused`). A harness written from either document fails all 31 vectors. | `money-compare.json` carries `less/equal/greater/refused`; §03 §4.3 names `schema-type-mismatch`. Confirmed. | **No**, but cheap, and it costs a newcomer a whole file. |
